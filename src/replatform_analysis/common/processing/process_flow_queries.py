...
def process_flows():
    ...
    def download_report(replatform):
        ...
        df['Entry_New'] = [[] if x == 'Entry' else x[1:-1].split(',') for x in df['Entry']] 
        ...
        df['Entry_New2'] = df.apply(lambda x: append_entry(x['Entry_New'],x['Page Type']), axis=1)
        ...
        def article_article(row):
            if len(row['Entry_New']) > 1:
                if row['Entry_New'][-1] == 'article' and row['Entry_New'][-2] == 'article':
                    return True
                else:
                    return False
            else:
        ...
        def article_home_article(row):
            if len(row['Entry_New']) > 2:
                if row['Entry_New'][-1] == 'article' and row['Entry_New'][-2] == 'home' and row['Entry_New'][-3] == 'article':
                    return True
                else:
                    return False
            else:
        ...
        df_entry_g = df_entry.groupby(by = ['Date','sys_env'])['Visits'].sum().reset_index()
        df_entry_g.columns = ['Date','sys_env','Entries']
        df_art_art = df_f[df_f['Article_Article']].copy()
        df_art_art_g = df_art_art.groupby(by = ['Date','sys_env'])['Visits'].sum().reset_index()
        df_art_art_g.columns = ['Date','sys_env','Article-Article']
        df_art_home_art = df_f[df_f['Article_Home_Article']].copy()
        df_art_home_art_g = df_art_home_art.groupby(by = ['Date','sys_env'])['Visits'].sum().reset_index()
        df_art_home_art_g.columns = ['Date','sys_env','Article-Home-Article']

        df_output = pd.merge(df_entry_g,df_art_art_g, on = ['Date','sys_env'],how='left')
        df_output = pd.merge(df_output,df_art_home_art_g, on = ['Date','sys_env'],how='left')

        df_output['System Environment'] = df_output['sys_env'].map(sys_env_mapping)
        return df_output
    ...
    biq_query_upload_drop(client,df_merged, dataset_id, output_table_id, flow_processed_bq_schema)

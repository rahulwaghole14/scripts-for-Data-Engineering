...
def biq_query_upload(client, df, dataset_id, table_id, schema):
    ...
    job_config = bigquery.LoadJobConfig(autodetect=True,schema=schema, source_format=bigquery.SourceFormat.CSV)
    ...
    df.columns=df.columns.str.replace('-','',regex=False)
    df.columns=df.columns.str.replace(')','',regex=False)
    job = client.load_table_from_dataframe(df, dimension_table_id, job_config=job_config)
    output = job.result()
    logging.info(output)

def biq_query_upload_drop(client, df, dataset_id, table_id, schema):
    dimension_table_id = dataset_id + '.' + table_id
    job_config = bigquery.LoadJobConfig(autodetect=True,schema=schema, source_format=bigquery.SourceFormat.CSV,write_disposition="WRITE_TRUNCATE")
    df.columns=df.columns.str.replace('(','',regex=False)
    df.columns=df.columns.str.replace(')','',regex=False)
    job = client.load_table_from_dataframe(df, dimension_table_id, job_config=job_config)
    output = job.result()

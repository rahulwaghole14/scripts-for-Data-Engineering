...
def get_dimension_name(dimension_id, rsid="fairfaxnzhexa-new-replatform"):
    request_url = BASE_URL + 'dimensions/'
    response = requests.get(
                url=request_url + dimension_id, headers=REQUEST_HEADERS, params  = {"rsid": rsid})
#                url=request_url + dimension_id, headers=REQUEST_HEADERS, params  = {"rsid": "fairfaxnz-hexaoverall-production"})
    return response.json()['name']


def get_metric_name(metric_id, rsid="fairfaxnzhexa-new-replatform"):
    request_url = BASE_URL + 'metrics/'
    response = requests.get(
                url=request_url + metric_id, headers=REQUEST_HEADERS, params  = {"rsid": rsid})
    return response.json()['name']

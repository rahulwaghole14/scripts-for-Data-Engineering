...
from common.oberon_queries.queries.logged_in_UVs_query import srp_logged_in_uvs_query, old_logged_in_UVS
...
def call_logged_in_UVs_report(replatform = True):
    client = create_bq_client()
    dataset_id = 'hexa_replatform_analysis'
    table_id = 'logged_in_UVs'
    logging.info(f'Running {table_id}.')
    if replatform:
        numdays = 7
        query = srp_logged_in_uvs_query
        query_time_period = 1
    else:
        numdays = 180
        query = old_logged_in_UVS
        query_time_period = 1

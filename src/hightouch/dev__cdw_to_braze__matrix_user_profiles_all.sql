...
-- hightouch name: {dev|prod}__cdw_to_braze__matrix_user_profiles_all
...
-- EXEC stage.[matrix].[load_braze_user_profiles_all]

CREATE OR ALTER PROCEDURE [matrix].[load_braze_user_profiles_all]
AS
BEGIN

    ;
    IF EXISTS (SELECT * FROM sys.tables WHERE name = 'hightouch_user_profiles_all' AND schema_id = SCHEMA_ID('matrix'))
    DROP TABLE stage.matrix.hightouch_user_profiles_all;
    ...
    AS (
        SELECT c.hash_key AS primary_key
            ...
            ,CASE 
                ...
                ELSE NULL
                END AS country
            ,lower(trim(coalesce(i.email, i2.email, c.PersContact3))) AS email
            ...
            ,CASE 
                WHEN i.user_id IS NOT NULL
                    OR i2.user_id IS NOT NULL
                    THEN 1
                ELSE 0
                END AS has_idm_account
        ...
        LEFT JOIN stage.idm.drupal__user_profiles i2 ON (i2.marketing_id = c.marketing_id)
    ...
    SELECT primary_key
        ...
        ,MAX(has_idm_account) AS has_idm_am_account
        ,CASE 
    ...
    INTO stage.matrix.hightouch_user_profiles_all
    FROM sub_active

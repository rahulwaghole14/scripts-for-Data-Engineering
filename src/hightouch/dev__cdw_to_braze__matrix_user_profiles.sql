-- use stage;
-- exec idm.load_braze_subs;


-- -- after create view use hightouch query

-- hightouch name: {dev|prod}__cdw_to_braze__matrix_user_profiles

-- SELECT -- TOP (20000) -- limit dev
--       [primary_key]
...
--   FROM [stage].[matrix].[hightouch_user_profiles_no_match]
...
--EXEC stage.matrix.load_braze_user_profiles_no_match

CREATE OR ALTER PROCEDURE [matrix].[load_braze_user_profiles_no_match]
AS
BEGIN

    ;
    IF EXISTS (SELECT * FROM sys.tables WHERE name = 'hightouch_user_profiles_no_match' AND schema_id = SCHEMA_ID('matrix'))
    DROP TABLE stage.matrix.hightouch_user_profiles_no_match;

    ;
    -- with idm_data as (
    --     select distinct subscriber_id from stage.idm.drupal__user_profiles where subscriber_id is not null
    -- )
    ...
    AS (
        ...
        ,subscribers
    AS (
        SELECT -- top(30000) -- limit dev
            c.hash_key AS primary_key
        ...
        LEFT JOIN stage.idm.drupal__user_profiles i ON i.subscriber_id = c.subs_id --  OR i.marketing_id = c.marketing_id
        ...
        -- WHERE DoNotCall = 0
        WHERE i.subscriber_id IS NULL -- need to seperate has because of marketing_id clash (email)
            -- and c.subs_id = 3882847
            -- if a matrix user has same marketing_id we should not overwrite with matrix email
        )
        ,rate_code
    AS (
    ...
    SELECT primary_key
        ...
        -- , MAX(has_idm_account) as has_idm_am_account -- for the matrix user_profiles without email sync in hightouch
        ,print__deceased
    INTO stage.matrix.hightouch_user_profiles_no_match -- new table
        --  select count(*) from stage.matrix.hightouch_user_profiles_no_match
    FROM sub_active

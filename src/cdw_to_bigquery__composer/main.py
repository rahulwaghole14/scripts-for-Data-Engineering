...
def load_config():
    ...
    config = {
        ...
        "destination_table": 'cdw_stage.composer__articles',
        "destination_table_tags": 'cdw_stage.composer__tags'
    }
    return config

if __name__ == "__main__":
    logger('composer_articles')
    try:
        config = load_config()
        ...
        QUERY = "SELECT * FROM stage.composer.composer__articles"
        QUERY_TAGS = "SELECT * FROM stage.composer.tags"
        CHUNK_SIZE = 50000
        FIRST_CHUNK = True

        # do composer__articles table
        ...
        # do composer tags table
        for chunk in pd.read_sql(QUERY_TAGS, engine, chunksize=CHUNK_SIZE):
            logging.info(f"Received chunk of size {len(chunk)}")

            IF_EXISTS = 'replace' if FIRST_CHUNK else 'append'
            to_gbq(chunk, config['destination_table_tags'], project_id=config['project_id'], if_exists=IF_EXISTS, credentials=credentials)
            FIRST_CHUNK = False

...
def load_config():
    ...
    config = {
        ...
        "destination_table": 'adw_stage.adbook__adrevenue',
    }
    return config
...
if __name__ == "__main__":
    ...
    try:
        ...
        QUERY = "SELECT * FROM [dbo].[AdBookNavigaHistoric]"

        # Output CSV file path
        csv_file_path = 'adbook_datanew.csv'
...
# SELECT count(*) FROM [FFXDW].[dbo].[AdBookNavigaHistoric]; -- 114297
# -- historic data for genera (one off migration)
# SELECT count(*) FROM [FFXDW].[dbo].[GeneraNavigaHistoric]; -- 868670
...
# SELECT count(*) FROM [FFXDW].[dbo].[NavigaTest]; -- 133611
# -- naviga accounts to genera accounts mapping table (one off migration)
# SELECT count(*) FROM [FFXDW].[dbo].[NavigaAccountMapping]; -- 5533

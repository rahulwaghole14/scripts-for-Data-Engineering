...
class Query(object):
    ...
    def __init__(self, suite_id):
        ...
        self._metrics_container_filter = None  # deepak added on 2020/03/04
    ...
    def filter(self, dimension_filter):
        ...
        if dimension_filter and not isinstance(
    ...
    def compile(self):
        ...
        global_filters = [
            {"type": "dateRange", "dateRange": self._compose_date_range()}
        ]
        ...
        self._translation = {
            ...
            "metricContainer": {
                "metrics": self._compose_metric_container(),
                "metricFilters": self._metrics_container_filter,
            },
    ...
    ##        metrics = []
    ##
    ##        for metric in self._metrics: # updated by deepak on 2020/03/05
    ##            if self._metrics_container_filter: #
    ...
    ##                metrics.append({'columnId': str(index) if not metric.column_id else str(metric.column_id),
    ##                            'id': metric.id,
    ##                            'filters': [f.id for f in metric.filters]}) # deepak added 2020/03/04
    ##
    ##            index += 1
    ##
    ##        return metrics

    def _compose_metric_container(
        self,
    ):  # deepak trying to revise on 2020/05/20
        if not self._metrics:
            raise QueryError("metrics must be specified")
        ...
        for metric in self._metrics:  # updated by deepak on 2020/03/05
            if self._metrics_container_filter:  #
                ##                print("length of metrics container: {0} and metrics: {1}".format(len(self._metrics_container_filter),len(self._metrics)))
                if len(self._metrics_container_filter) > len(self._metrics):
                    temp_index = [
                        str(index + i * len(self._metrics))
                        for i in range(
                            int(
                                len(self._metrics_container_filter)
                                / len(self._metrics)
                            )
                        )
                    ]
                ...
                )  # deepak added 2020/03/04

            index += 1

        return metrics

    ##    def metrics_containers_filters(self, dimensions): # deepak added 2020/03/04
    ##
    ##        if not isinstance(dimensions, dict):
    ##            raise QueryError("Metric Filters for Metric Containers should be dict")
    ##
    ##        self._metrics_container_filter = []
    ##        index = 0
    ##
    ##        for metric in self._metrics:
    ##
    ##            temp_dict = {"id":str(index) if not metric.column_id else str(metric.column_id)}
    ##            temp_dict.update(dimensions)
    ##            self._metrics_container_filter.append(temp_dict)
    ##
    ##            index += 1
    ##
    ##        del temp_dict
    ...
    def metrics_containers_filters(
        self, dimensions
    ):  # deepak revising on 2020/05/20

        ##        {'type': 'breakdown', 'dimension': ['variables/prop11', 'variables/daterangeday'], 'itemId': ['1257564797', '1200418']}

        if not isinstance(dimensions, dict):
            raise QueryError(
                "Metric Filters for Metric Containers should be dict"
            )

        self._metrics_container_filter = []
        index = 0

        if isinstance(dimensions["dimension"], list):
            for i, val in enumerate(dimensions["dimension"]):
                ##                temp_dimension = dimensions['dimension'][i]
                ##                temp_itemID = dimensions['itemId'][i]
                for metric in self._metrics:
                    temp_dict = {
                        "id": str(index),
                        "type": "breakdown",
                        "dimension": dimensions["dimension"][i],
                        "itemId": dimensions["itemId"][i],
                    }
                    ##                    temp_dict.update(dimensions)
                    self._metrics_container_filter.append(temp_dict)
    ...
    def _compose_settings(self):
        ...
        # settings.update({"dimensionSort": "asc"})

        if not self._sort_mode:
            self._sort_mode = Query.Sort.asc

        # settings.update({'dimensionSort': Query._sort_map[self._sort_mode]})
...
class MetricFilter(object):
    ...
    def __init__(self, filter_type, value, id=None):
        self._id = b2a_hex(urandom(8)) if not id else id
        self._filter_type = filter_type
        # Attr value data structure depends on filter type (e.g.: dimension, granularity, etc)

        if filter_type == FilterType.breakdown and not isinstance(
            value, tuple
        ):
            raise QueryError(
    ...
    def compile(self):
        metric_filter_map = {"id": self._id, "type": self._compose_type()}

        if self._filter_type == FilterType.breakdown:
            metric_filter_map.update(
                {
                    self._compose_type_key(): self._value[0],
                    "itemIds": self.value[1],
                }
            )
        else:
            metric_filter_map.update({self._compose_type_key(): self._value})
            ...
            if isinstance(clause, tuple):
                ...
                for index, item in enumerate(clause):
                    if not index:
                        output.append(
                            " {} ( {}".format(
                                Clause._conjunction_map[item._conjunction],
                                item.compile(),
                            )
                        )
                        continue

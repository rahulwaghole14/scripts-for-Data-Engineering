...
def request_composer_conv_report(url, app_id, app_api_token, exp_id, start_date, end_date): # pylint: disable=too-many-arguments,too-many-locals
    """
    ...
    try:
        ...
        if response.status_code == 200:
            logging.info(
                ...
                exp_id, start_date, end_date
                )
            return response.json()
        logging.error(
            "Failed to request report for Experience ID %s: HTTP %s",
            exp_id, response.status_code
            )
        return {
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if job_status in ['FAILED', 'INTERNAL_ERROR']:
                    logging.error("Report generation failed with status: %s", job_status)
                    return data
                logging.info("Report generation in progress. Waiting before next check...")
                time.sleep(check_interval)
            else:
                logging.error("Failed to check report status: HTTP %s", response.status_code)
                return None
        except Exception as error: # pylint: disable=broad-except
            logging.error("Error checking report status: %s", error)
            return None

def download_report_and_load_csvs_to_dataframes(app_id, app_api_token, export_id):
    """
    ...
    if response.status_code == 200:
        ...
        if download_url:
            ...
            if file_response.status_code == 200:
                ...
                return dataframes
            logging.info("Failed to download the report: HTTP %s", file_response.status_code)
        logging.info("Download URL not found in the response.")
    else:
        logging.info("Failed to obtain download URL: HTTP %s", response.status_code)
    return dataframes

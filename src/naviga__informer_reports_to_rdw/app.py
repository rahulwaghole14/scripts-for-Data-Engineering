...
def upload_csv_to_bigquery(dataset_id, table_id, csv_path):
    try:
        ...
        dataframe['Campaign ID'] = dataframe['Campaign ID'].str.replace(',', '').astype(float)
        ...
        logging.info("Uploaded %s to %s.%s in BigQuery", csv_path, dataset_id, table_id)
...
if csv_file_path:
    upload_csv_to_bigquery(dataset_id, table_id, csv_file_path)  # Upload to BigQuery

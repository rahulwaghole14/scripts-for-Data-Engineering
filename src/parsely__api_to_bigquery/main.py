...
def get_max_date_from_bigquery(table_id, client):
    ...
    try:
        ...
        logging.warning(f"Could not get max_date from {table_id}, assuming table does not exist. {e}")
        return None
...
def append_to_bigquery(data, table_id, client, cred, project_id):
    try:
        ...
        to_gbq(data, destination_table=table_id, if_exists='replace', project_id=project_id, credentials=cred)
        return

    # Append data to the existing table
    to_gbq(data, destination_table=table_id, if_exists='append', project_id=project_id, credentials=cred)
...
def get_parsely_data(full_url, start_date, end_date, limit, key, secret):
    ...
    for single_date in pd.date_range(start_date, end_date):
        ...
        page_number = 1
        while True:
            url = f"{full_url}?apikey={key}&secret={secret}&pub_date_start={pub_date}" \
...
if __name__ == "__main__":
    ...
    TABLE_ID = f"{PROJECT_ID}.{DATASET_ID}.parsely_api__tags"
    START_DATE = "2023-01-01"
    ENDPOINT = "analytics/posts"
    FULL_URL = f"http://api.parsely.com/v2/{ENDPOINT}"
    LIMIT = 1000

    encoded_cred = os.environ.get("GOOGLE_CLOUD_CRED_BASE64")
    ...
    if not all([google_cred, parsely_key, parsely_secret]):
    ...
    start_date_to_query = pd.Timestamp(max_date) + pd.Timedelta(days=1) if max_date else pd.Timestamp(START_DATE)
    # 7 days only (for testing)
    # end_date_to_query = start_date_to_query + pd.Timedelta(days=7)
    end_date_to_query = pd.Timestamp(pd.to_datetime("today").strftime('%Y-%m-%d'))

    if start_date_to_query <= end_date_to_query:
        parsely_data = get_parsely_data(FULL_URL, start_date_to_query, end_date_to_query, LIMIT, parsely_key, parsely_secret)

        # Convert all list columns to JSON strings
        for col in parsely_data.columns:
            if parsely_data[col].apply(type).eq(list).any():
                parsely_data[col] = parsely_data[col].apply(json.dumps)

        # Serialize all dict columns to JSON strings
        for col in parsely_data.columns:
            if parsely_data[col].apply(type).eq(dict).any():
                parsely_data[col] = parsely_data[col].apply(json.dumps)

        if not parsely_data.empty:
            append_to_bigquery(parsely_data, TABLE_ID, client, credentials, PROJECT_ID)

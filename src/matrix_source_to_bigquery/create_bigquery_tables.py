...
def map_data_types(sql_server_type):
    ...
    return mapping.get(sql_server_type.lower(), "STRING")  # Default to STRING if type unknown

def extract_schema_from_sql_server(server, portnumber, database, username, password, tables_to_extract,columns_to_include):
    ...
    try:
        ...
        for schema_name, table_name in tables_to_extract:
            ...
            if table_name in columns_to_include:
                ...
                filtered_columns = [col for col in columns if col[0] in columns_to_include[table_name]]
                schema_info[full_table_name] = [(col[0], map_data_types(col[1])) for col in filtered_columns]
            else:
                schema_info[full_table_name] = [(col[0], map_data_types(col[1])) for col in columns]
...
def create_bigquery_table(client, dataset_name, full_table_name, schema_info, hash_key_tables):
    try:
        ...
        if full_table_name in hash_key_tables:
            schema.append(bigquery.SchemaField('hash_key', 'STRING', mode="NULLABLE"))
        ...
        try:
            ...
            logging.warning(f"Table {full_table_name} already exists in dataset {dataset_name}.")
    except Exception as e:
        logging.error(f"Error creating table {full_table_name} in dataset {dataset_name}: {e}")
...
def create_bigquery_dataset(client, dataset_name, location):
    ...
    try:
        ...
        try:
            ...
            logging.info(f"Created dataset {client.project}.{dataset_name} in {location}")
        except Exception as e:
...
def create_batch_metadata_table(client, dataset_name):
    ...
    schema = [
        ...
        bigquery.SchemaField('last_processed_date', 'TIMESTAMP', mode='NULLABLE')
    ]
    table = bigquery.Table(table_ref, schema=schema)
    try:
        created_table = client.create_table(table)
...
def add_column_to_bigquery_table(client, dataset_name, full_table_name):
    ...
    if not any(field.name == 'update_time' for field in table.schema):
        ...
        new_schema.append(bigquery.SchemaField("update_time", "TIMESTAMP", mode="NULLABLE"))
        table.schema = new_schema  # Update the schema
        table = client.update_table(table, ["schema"])  # Make an API request to update the table schema
...
def main():
    ...
    hash_key_tables = [f"{schema}.{table}" if schema != 'dbo' else table for schema, table in tables_to_extract if table in hash_key_tables]

    schema_info = extract_schema_from_sql_server(server, portnumber, database, username, password, tables_to_extract, columns_to_include)
    for schema, table in tables_to_extract:
        ...
        if full_table_name in schema_info:
            create_bigquery_table(client, dataset_name, full_table_name, schema_info, hash_key_tables)

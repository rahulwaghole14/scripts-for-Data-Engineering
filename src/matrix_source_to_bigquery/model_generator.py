...
def sql_type_to_python_type(sql_type: str, is_nullable: bool):
    ...
    python_type, extra = mapping.get(sql_type.lower(), (str, ...))  # Default to str if unknown
    if is_nullable:
...
def fetch_and_create_dynamic_model(cursor, schema_name: str, table_name: str):
    ...
    fields = {
        row.COLUMN_NAME: sql_type_to_python_type(row.DATA_TYPE, row.IS_NULLABLE == 'YES')
    ...
    DynamicModel = create_model(
        model_name,
        **{field_name: (field_type, None) for field_name, (field_type, _) in fields.items()},
        __base__=BaseModel
    )

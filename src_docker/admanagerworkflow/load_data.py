...
from .bin.run_saved_report import (
    run_report,
...
def setup_logger():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(log_directory, "data_load_log.log")),
...
service_account_info = config["gam_keyfile"]

# Write the key to a temporary file for use in docker
with tempfile.NamedTemporaryFile(
    mode="w+", suffix=".json", dir="/tmp", delete=False
) as temp_key_file:
    json.dump(service_account_info, temp_key_file)
    temp_key_file.flush()
...
oauth2_client = oauth2.GoogleServiceAccountClient(
    ...
    sub=service_account_info.get("client_email"),
)

# Initialize the Google Ad Manager client with the credentials
gam_client = ad_manager.AdManagerClient(
    oauth2_client=oauth2_client,
    application_name=config["gam_yaml"]["application_name"],
    network_code=config["gam_yaml"]["network_code"],
)

# Get the ReportService
report_service = gam_client.GetService("ReportService", version=config["api_version"])
...
def delete_data_for_date_range(client, table_id, start_date_del, end_date_del):
    ...
    logging.info("Deleted data from %s to %s in table %s", start_str, end_str, table_id)
...
def load_data_for_date_range(
    ...
    if not skip_deletion and table_exists(bq_client, table_id):
        delete_data_for_date_range(bq_client, table_id, start_date_load, end_date_load)

    current_date = start_date_load
    while current_date <= end_date_load:
        logging.info("Processing data for date: %s", current_date.strftime("%Y-%m-%d"))
        attempts = 0
            ...
            try:
                if not data_already_loaded(bq_client, table_id, current_date):
                    ...
                    if not dataframe.empty:
                        if not table_already_exists:  # Create table if it doesn't exist

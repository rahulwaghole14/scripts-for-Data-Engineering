...
from common.oberon_queries.queries.referral_traffic import srp_referral_search, old_referral_traffic
...
def call_search_referral_traffic_report(replatform = True):
    client = create_bq_client()
    dataset_id = 'hexa_replatform_analysis'
    table_id = 'referral_traffic_counts' 
    logging.info(f'Running {table_id}.')
    if replatform:
        numdays = 7
        query = srp_referral_search
        day_limit = 1
        query_time_period = 1

...
from common.oberon_queries.queries.platform_metrics import srp_platform, old_platform
...
def call_platform_report(replatform = True):
    client = create_bq_client()
    dataset_id = 'hexa_replatform_analysis'
    table_id = 'platform_analysis'
    ...
    for date_ in dateList:
        ...
        if date_start not in bq_date_list:
            ...
            df['date'] = date_start
            df.columns = ['Day', 'UVs - Domestic Traffic NZ',
                                'UVs - Android App',
                                'UVs - iOS App',
                                'UVs - Mobile Site',
                                'UVs - Desktop Site',

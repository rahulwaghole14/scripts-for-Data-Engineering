...
from .validator.data_validator_base import naviga_data_targets, DateTimeEncoder
        ...
        if response.status_code == 200:
            ...
            for chunk in response.iter_content(chunk_size=chunk_size):
                if chunk:
                    ...
                    if current_time - last_log_time >= log_interval:
                        ...
                        logging.info(
                            f"Downloaded {downloaded_size / (1024 * 1024):.2f} MiB at "
            ...
            for attempt in range(retries):
                try:
                    ...
                    for item in reader:
                        try:
                            validated_model = naviga_data_targets(**item)
            ...
            schema = pydantic_model_to_bq_schema(naviga_data_targets)
...
def naviga_run_targets():
    ...
    if data is not None:
        ...
        table_id = "naviga__targets"  # Adding bq to differ the new tables with the current ones
        json_data = json.dumps(data, cls=DateTimeEncoder)

        load_data_to_bigquery_truncate(
            client, dataset_id, table_id, json.loads(json_data), schema
        )

...
def download_dataframe_no_pagelimit(resource, df):
    ...
    while flag:
        try:
            url = base_url + "/api/v1/{0}?page={1}".format(
    ...
    logging.info(
        'total execution time for "{0}" = {1} sec'.format(
            resource, stop - start
        )
    )
...
def download_dataframe_with_pagelimit(resource, df, pagelimit):
    ...
    df.reset_index(drop=True, inplace=True)
    logging.info(
        'Total download time for "{0}" = {1} sec'.format(
            resource, stop - start
        )
    )
...
def ensure_dataset_in_australia(client, dataset_id):
    ...
    try:
        ...
        logging.info(
            f"Dataset '{dataset_id}' not found. Creating in Australia."
        )
...
def process_presspatron_data():
    google_sec = get_secret("datateam_google_prod")
    bigquery_secret = json.loads(google_sec)
    ...
    schema_users = [
        ...
        bigquery.SchemaField(
            "newsletterSubscribed", bigquery.enums.SqlTypeNames.STRING
        ),
    ...
    schema_subscriptions = [
        ...
        bigquery.SchemaField("updatedAt", bigquery.enums.SqlTypeNames.STRING),
        bigquery.SchemaField(
            "subscriptionId", bigquery.enums.SqlTypeNames.STRING
        ),
        bigquery.SchemaField("userId", bigquery.enums.SqlTypeNames.STRING),
        bigquery.SchemaField(
            "cancellationAt", bigquery.enums.SqlTypeNames.STRING
        ),
        bigquery.SchemaField(
            "grossAmount", bigquery.enums.SqlTypeNames.STRING
        ),
        bigquery.SchemaField("frequency", bigquery.enums.SqlTypeNames.STRING),
        bigquery.SchemaField(
            "subscriptionStatus", bigquery.enums.SqlTypeNames.STRING
        ),
        bigquery.SchemaField(
            "rewardSelected", bigquery.enums.SqlTypeNames.STRING
        ),
    ...
    schema_transactions = [
        ...
        bigquery.SchemaField("updatedAt", bigquery.enums.SqlTypeNames.STRING),
        bigquery.SchemaField(
            "transactionId", bigquery.enums.SqlTypeNames.STRING
        ),
        bigquery.SchemaField(
            "grossAmount", bigquery.enums.SqlTypeNames.STRING
        ),
        bigquery.SchemaField(
            "cardIssueCountry", bigquery.enums.SqlTypeNames.STRING
        ),
        bigquery.SchemaField("userId", bigquery.enums.SqlTypeNames.STRING),
        bigquery.SchemaField(
            "subscriptionId", bigquery.enums.SqlTypeNames.STRING
        ...
        bigquery.SchemaField("frequency", bigquery.enums.SqlTypeNames.STRING),
        bigquery.SchemaField(
            "processorCreditCardFee", bigquery.enums.SqlTypeNames.STRING
        ),
        ...
        bigquery.SchemaField(
            "processorBankTransferFee", bigquery.enums.SqlTypeNames.STRING
        ),
        bigquery.SchemaField(
            "pressPatronCommission", bigquery.enums.SqlTypeNames.STRING
        ),
        bigquery.SchemaField(
            "rewardSelected", bigquery.enums.SqlTypeNames.STRING
        ),
        ...
        bigquery.SchemaField("urlSource", bigquery.enums.SqlTypeNames.STRING),
        bigquery.SchemaField(
            "paymentStatus", bigquery.enums.SqlTypeNames.STRING
        ),
        bigquery.SchemaField(
            "pressPatronCommissionSalesTax", bigquery.enums.SqlTypeNames.STRING
        ),
        bigquery.SchemaField("load_dts", bigquery.enums.SqlTypeNames.STRING),
    ]
    ...
    for file in files:
        table_id = f"{project_id}.{dataset_id}.{file}"
        try:
            table = client.get_table(table_id)
            logging.info(
                f"Table '{table_id}' already exists and has {table.num_rows} rows."
            )
            ...
            if table.num_rows == 0:
                logging.info(f"Table '{table_id}' is empty. Appending data.")
                df_new["load_dts"] = str(pd.Timestamp.now())
                append_dataframe_to_bigquery(
                    df_new, client, table_id, bq_schema[file]
                )
            else:
                ...
                if not df_old.empty:
                    if "load_dts" in df_old.columns:
                        ...
                        print(
                            "Column 'load_dts' does not exist in the DataFrame."
                        )
                    print(df_old.info())
                    df_all = df_new.merge(
            ...
            logging.info(
                f"Table '{table_id}' is not found. Creating new table."
            )
            df = pd.DataFrame()

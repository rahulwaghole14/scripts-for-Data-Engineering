...
with source as (
      select * from {{ source('adbook', 'adbook__adrevenue') }}
),
...
renamed as (
    ...
    select
        ...
        TIMESTAMP({{ adapter.quote("month_start_date") }}) AS RECORD_LOAD_DTS,
        TIMESTAMP({{ adapter.quote("month_start_date") }}) AS EFFECTIVE_FROM,

...
WITH source AS (
    SELECT *, _TABLE_SUFFIX FROM {{ source('adobe_analytics', 'data_*') }}
...
referrer_type AS (
    SELECT id, any_value(name) as NAME FROM {{ source('adobe_analytics', 'referrer_type') }} group by 1
),
event_lookup AS (
    SELECT id, any_value(name) as NAME  FROM {{ source('adobe_analytics', 'event') }} group by 1
),
 ...
 renamed as (
    select
        ...
        {{ adapter.quote("mobiledayofweek") }} AS MOBILE_DAY_OF_WEEK,  --repltform
        ...
        {{ adapter.quote("mobilehourofday") }} AS MOBILE_HOUR_OF_DAY, --repltform
        ...
        {# {{ adapter.quote("post_mobiledayofweek") }} AS POST_MOBILE_DAY_OF_WEEK, --repltform
        ...
        {{ adapter.quote("prop11") }} AS PROP11, --masthead & repltform
        {# {{ adapter.quote("prop12") }},
        {{ adapter.quote("prop13") }}, #}
        {{ adapter.quote("prop14") }} AS PROP14, --masthead & repltform

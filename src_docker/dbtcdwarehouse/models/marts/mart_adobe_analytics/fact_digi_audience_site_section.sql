...
PageViews AS (
    ...
    WHERE
        ...
        AND (
            {% if is_incremental() %}
                DATE(LOAD_DATE) >= DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY)
            {% else %}
                DATE(LOAD_DATE) >= '2024-01-01'  -- Fixed date for full runs
            {% endif %}
        )
...
INNER JOIN PageViews pv ON (uv.DAY = pv.DAY AND uv.SITE_SECTION = pv.SITE_SECTION)
INNER JOIN Visits v ON (uv.DAY = v.DAY AND uv.SITE_SECTION = v.SITE_SECTION)

...
try:
    ...
    dbt_command = ["dbt", "run", "--select", "+tag:bq_matrix"]
    # dbt_command = ["dbt", "build", "--select", "+tag:bq_matrix"] # build introduce run test before materialise downstream models
    # dbt_command = ["dbt", "run", "--models", "int_matrix__to_braze_user_profile"] # build introduce run test before materialise downstream models

...
try:
    ...
    dbt_command = ["dbt", "run", "--select", "+tag:qualtrics_surveys"]
    # dbt_command = ["dbt", "build", "--select", "+tag:qualtrics_news_hub"] # build introduce run test before materialise downstream models
    # dbt_command = ["dbt", "run", "--models", "int_news_hub__to_braze_user_profile"] # build introduce run test before materialise downstream models

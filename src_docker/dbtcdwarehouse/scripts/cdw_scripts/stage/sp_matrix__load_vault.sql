...
GO
CREATE OR ALTER PROCEDURE [matrix].[load_vault] AS

	-- Usage
	-- exec [stage].[matrix].[load_vault];

	-- matrix vault (full list)
	-- select top(10) * from [vault].[raw].[link_cancellation] where record_source = 'MATRIX'
	-- select top(10) * from [vault].[raw].[link_subscription] where record_source = 'MATRIX'
	-- select top(10) * from [vault].[raw].[hub_cancellation_reason] where record_source = 'MATRIX'
    -- select top(10) * from [vault].[raw].[hub_period] where record_source = 'MATRIX'
	-- select top(10) * from [vault].[raw].[hub_product] where record_source = 'MATRIX'
	-- select top(10) * from [vault].[raw].[hub_customer] where record_source = 'MATRIX'
	-- select top(10) * from [vault].[raw].[hub_service] where record_source = 'MATRIX'
	-- select top(10) * from [vault].[raw].[sat_service] where record_source = 'MATRIX'
	-- select top(10) * from [vault].[raw].[sat_product] where record_source = 'MATRIX'
	-- select top(10) * from [vault].[raw].[sat_customer] where record_source = 'MATRIX'
	-- select top(10) * from [vault].[raw].[sat_period] where record_source = 'MATRIX'
	-- select top(10) * from [vault].[raw].[sat_cancellation_reason] where record_source = 'MATRIX'
	-- select top(10) * from [vault].[raw].[sat_subscription] where record_source = 'MATRIX'
	-- select top(10) * from [vault].[raw].[sat_cancellation] where record_source = 'MATRIX'
	...
	WITH phone_numbers AS (
		SELECT
			cn.ObjectPointer
			, MAX(CASE WHEN cn.ContactType = 'HOME' THEN CONCAT(cn.AreaCode, cn.ContactNumber) END) AS HOME
			, MAX(CASE WHEN cn.ContactType = 'WORK' THEN CONCAT(cn.AreaCode, cn.ContactNumber) END) AS WORK
			, MAX(CASE WHEN cn.ContactType = 'MOBILE' THEN CONCAT(cn.AreaCode, cn.ContactNumber) END) AS MOBILE
			, MAX(cn.DoNotCall) as DoNotCall
		FROM
			[stage].[matrix].[tbl_contact_number] cn
		GROUP BY
			cn.ObjectPointer
	)

	, subscribers AS (
		SELECT DISTINCT [subscriber].hash_key
			, [subscriber].subs_id
			, [subscriber].subtype_id
			, [tbl_person].title
			, [tbl_person].FirstName
			, [tbl_person].SurName
			, [tbl_person].PersContact3
	...
	, add_numbers AS (
		SELECT subscribers.hash_key
			, subscribers.subs_id
			, subscribers.subtype_id
			, subscribers.title
			, subscribers.FirstName
			, subscribers.SurName
			, subscribers.PersContact3
			, phone_numbers.HOME
			, phone_numbers.WORK
			, phone_numbers.MOBILE
			, phone_numbers.DoNotCall
			, subscribers.AddrLine3
			, subscribers.AddrLine4
			, subscribers.AddrLine5
			, subscribers.AddrLine6
			, subscribers.CountryName
			, subscribers.record_source
			, subscribers.record_load_dts
		FROM subscribers LEFT JOIN phone_numbers ON subscribers.ObjectPointer = phone_numbers.ObjectPointer
	)
	...
	, add_hash AS (
		...
		, substring(
			convert(char,hashbytes('MD5',
				concat_ws('|'
				, coalesce(add_numbers.title,'')
				, coalesce(add_numbers.FirstName,'')
				, coalesce(add_numbers.SurName,'')
				, coalesce(add_numbers.PersContact3,'')
				, coalesce(add_numbers.HOME,'')
				, coalesce(add_numbers.WORK,'')
				, coalesce(add_numbers.MOBILE,'')
				, coalesce(add_numbers.AddrLine3,'')
				, coalesce(add_numbers.AddrLine4,'')
				, coalesce(add_numbers.AddrLine5,'')
				, coalesce(add_numbers.AddrLine6,'')
				, coalesce(add_numbers.CountryName,'')
				)
				),2)
			,3,32) AS hash_diff
		, NULL AS external_id
	...
	MERGE INTO [stage].[matrix].[customer] AS Target
	USING (SELECT * FROM add_hash) AS Source
	ON Target.hash_key = Source.hash_key

	WHEN MATCHED AND Target.hash_diff <> Source.hash_diff THEN
		UPDATE SET
			Target.subs_id = Source.subs_id,
			Target.subs_type = Source.subtype_id,
			Target.title = Source.title,
			Target.FirstName = Source.FirstName,
			Target.SurName = Source.SurName,
			Target.PersContact3 = Source.PersContact3,
			Target.HOME = Source.HOME,
			Target.WORK = Source.WORK,
			Target.MOBILE = Source.MOBILE,
			Target.DoNotCall = Source.DoNotCall,
			Target.AddrLine3 = Source.AddrLine3,
			Target.AddrLine4 = Source.AddrLine4,
			Target.AddrLine5 = Source.AddrLine5,
			Target.AddrLine6 = Source.AddrLine6,
			Target.CountryName = Source.CountryName,
			Target.record_source = Source.record_source,
			Target.record_load_dts = Source.record_load_dts,
			Target.hash_diff = Source.hash_diff

	WHEN NOT MATCHED BY TARGET THEN
		INSERT (
			hash_key, subs_id, subs_type, title, FirstName, SurName, PersContact3, HOME, WORK, MOBILE, DoNotCall, AddrLine3, AddrLine4, AddrLine5, AddrLine6, CountryName, record_source, record_load_dts, hash_diff, external_id
		)
		VALUES (
			Source.hash_key, Source.subs_id, Source.subtype_id, Source.title, Source.FirstName, Source.SurName, Source.PersContact3, Source.HOME, Source.WORK, Source.MOBILE, Source.DoNotCall, Source.AddrLine3, Source.AddrLine4, Source.AddrLine5, Source.AddrLine6, Source.CountryName, Source.record_source, Source.record_load_dts, Source.hash_diff, Source.external_id
		)
	...
	-- 		convert(char,hashbytes('MD5',
	-- 			concat_ws('|'
	-- 			, coalesce([tbl_person].title,'')
	-- 			, coalesce([tbl_person].FirstName,'')
	-- 			, coalesce([tbl_person].SurName,'')
	-- 			, coalesce([tbl_person].PersContact3,'')
	-- 			, coalesce(contact.HOME,'')
	-- 			, coalesce(contact.WORK,'')
	-- 			, coalesce(contact.MOBILE,'')
	-- 			, coalesce([tbl_location].AddrLine3,'')
	-- 			, coalesce([tbl_location].AddrLine4,'')
	-- 			, coalesce([tbl_location].AddrLine5,'')
	-- 			, coalesce([tbl_location].AddrLine6,'')
	-- 			, coalesce([tbl_country].CountryName,'')
	-- 			)
	-- 		),2)
	-- 	,3,32) as hash_diff
	-- from [stage].[matrix].[subscriber]
	...
	-- 	from (
	-- 		select ObjectPointer
	-- 		, ContactType
	-- 		, max(concat(AreaCode,ContactNumber)) phone

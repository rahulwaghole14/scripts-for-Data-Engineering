...
SECRET_NAME = "brightcove"  # pragma: allowlist secret
secret_value = get_secret(SECRET_NAME)  # pylint: disable=invalid-name
data = json.loads(secret_value)

BRIGHTCOVE_DATA_URL = data["BRIGHTCOVE_DATA_URL"]
...
def get_destination_path_data(  # pylint: disable=too-many-arguments
    token: dict,
    date: str,
    brightcove_account_id: str,
    limit: int = 10,
    ...
    """
    Get get_destination_path_data from Brightcove API
    """
    ...
    try:
        ...
        logging.error("Error in get_destination_path_data: %s", e)
        return 0, []


def get_hourly_data(  # pylint: disable=too-many-arguments
    token: dict,
    date: str,
    brightcove_account_id: str,
    limit: int = 10,
    ...
    """
    Get get_hourly_data from Brightcove API
    """
    ...
    try:
        ...
        # Add the date field to each item
        for item in items:
            if "date_hour" in item:
                date_hour_str = item["date_hour"]
                date_hour = datetime.datetime.fromisoformat(date_hour_str)
                item["date"] = date_hour.date().strftime("%Y-%m-%d")

        return total, items

    except Exception as e:  # pylint: disable=broad-except
        logging.error("Error in get_hourly_data: %s", e)
        return 0, []

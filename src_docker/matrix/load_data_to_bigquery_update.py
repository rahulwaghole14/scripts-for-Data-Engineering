...
def timeout_handler(signum, frame):
    global should_exit
    should_exit = True
    logger.error("Script execution time exceeded the maximum allowed time. Initiating graceful shutdown.")
...
def check_exit():
    if should_exit:
        logger.info("Exiting script due to timeout or error.")
        sys.exit(1)

def parse_args():
    parser = argparse.ArgumentParser(description="Matrix data transfer to BigQuery")
...
class Watchdog(threading.Thread):
    def __init__(self, timeout, callback):
        ...
        self.reset()
        self.daemon = (
            True  # Allow the program to exit even if watchdog is still running
        )
        self.start()
...
def convert_to_serializable(value):
    if isinstance(value, datetime):
        ...
        logger.debug(
            f"String exceeds max length with size {len(value)} bytes. Truncating..."
        )
...
def prepare_data_for_bigquery(validated_data, include_hash_key=False):
    ...
    for row in validated_data:
        ...
        if include_hash_key:
            ...
            processed_row[
                "hash_key"
            ] = hash_key  # Add hash key to the row data only if required
...
def sanitize_table_name(schema_name, table_name):
    ...
    return (
        f"{schema_name}_{table_name}".replace(".", "_")
        if schema_name != "dbo"
        else table_name
    )
...
def update_last_processed_info(
    client, dataset_name, schema_name, table_name, last_processed_value
):
    bq_table_name = sanitize_table_name(schema_name, table_name)
...
def construct_merge_statement(
    ...
    columns = [
        field.name
        for field in schema
        if field.name not in ("update_time", "hash_key")
    ]  # Exclude control columns
    all_columns = columns + [
        "hash_key",
        "update_time",
    ]  # Include 'hash_key' and 'update_time' for INSERT and UPDATE
    ...
    on_condition = (
        f"ON TARGET.{primary_key_column} = SOURCE.{primary_key_column}"
        if primary_key_column
        else "ON TARGET.hash_key = SOURCE.hash_key"
    )
...
def upload_data_to_bigquery(
    ...
    while attempt < max_retries:
        try:
            ...
            if errors:
                logger.error(
                    f"Attempt {attempt+1}: Failed to upload data to BigQuery table {table_name}: {errors}"
                )
                attempt += 1
                time.sleep(retry_delay)  # Wait before retrying
            else:
                logger.debug(f"Uploaded {len(data)} rows to BigQuery table {table_name}.")
                return len(data)  # Return the number of rows uploaded
        except Exception as e:
            logger.error(
                f"Attempt {attempt + 1}: Exception during data upload to {table_name}: {str(e)}"
            )
            attempt += 1
            time.sleep(retry_delay)  # Wait before retrying
    ...
    logger.error(
        f"Failed to upload data to BigQuery table {table_name} after {max_retries} attempts."
    )
    return 0  # Indicate failure by returning 0


def load_full_refresh_data_to_bigquery(
    cursor, client, dataset_name, schema_name, table_name, batch_size=1000
):
    """
    ...
    """
    logger.debug(f"Starting full refresh for table {table_name}.")
    # Generate a dynamic Pydantic model based on the table schema
    dynamic_model = fetch_and_create_dynamic_model(
    ...
    while True:
        ...
        for row in rows:
            ...
            validated_row = validate_data_with_pydantic(
                dynamic_model, row_data
            )
            if validated_row:
                # If row is valid, prepare it for BigQuery
                prepared_row = prepare_data_for_bigquery(
                    [validated_row], include_hash_key=False
                )
                validated_data_for_bigquery.extend(prepared_row)
            else:
                # Log validation failure or take appropriate action
                logger.error(
                    f"Validation failed for row in table {table_name}: {row_data}"
                )

        # Upload validated and prepared data to BigQuery
        if validated_data_for_bigquery:
            success = upload_data_to_bigquery(
                client,
                dataset_name,
                bq_table_name,
                validated_data_for_bigquery,
            ...
            if success:
                logger.debug(f"Uploaded {len(validated_data_for_bigquery)} rows to {table_name}.")
            else:
                logger.error(
                    f"Failed to upload validated data for table {table_name}."
                )
...
def load_data_from_sql_server(
    ...
    if table_name == "tbl_RPT_TM1retail":
        ...
        where_clause = (
            f"{where_clause} AND {date_clause}"
            if where_clause
    ...
    if table_name == "tbl_ObjectLink":
        ...
        where_clause = (
            f"{where_clause} AND {link_clause}"
            if where_clause
            else f"WHERE {link_clause}"
        )

    order_by_clause = (
        f"ORDER BY {key_columns if key_columns else 'Retail_SaleDate'}"
    )
    ...
    while True:
        rows = cursor.fetchmany(batch_size)
        if not rows:
            logger.info(f"Completed fetching data for {table_name}. Total rows: {total_rows}")
            break
        total_rows += len(rows)
        elapsed_time = time.time() - start_time
        logger.debug(f"Fetched {total_rows} rows from {table_name} in {elapsed_time:.2f} seconds")
        yield rows
...
def update_bigquery_with_hash_keys(
    client,
    dataset_name,
    schema_name,
    table_name,
    ...
    merge_sql = construct_merge_statement(
        client,
        schema,
        dataset_name,
        bq_table_name,
    ...
    while attempt < max_retries:
        try:
            ...
            if hasattr(job, "num_dml_affected_rows"):
                logger.debug(f"{table_name} merged. Job ID: {job.job_id}, Rows affected: {job.num_dml_affected_rows}")

            else:
                logger.debug(
                    f"Data for {table_name} merged based on hash keys. Job ID: {job.job_id}, Rows affected: N/A"
                )

            return True
        except Exception as e:
            logger.error(
                f"Attempt {attempt + 1}: Failed to merge data for {table_name}: {e}"
            )
            attempt += 1
            time.sleep(retry_delay)  # Wait before retrying
            ...
            if attempt < max_retries:
                logger.info(
                    f"Retrying merge operation for {table_name} after {retry_delay} seconds..."
                )

    logger.error(
        f"Failed to merge data for {table_name} after {max_retries} attempts."
    )
    return False
...
def create_temp_table(client, dataset_name, schema_name, table_name, schema):
    ...
    logger.debug(f"Created temp table {table_id}")
...
def drop_temp_table(client, dataset_name, temp_table_name):
    ...
    logger.debug(f"Dropped temp table {table_id}")
...
def main():
    ...
    try:
        ...
        logger = logger(
            "matrix_data_transfer_to_bigQuery",
            os.path.join(os.path.dirname(os.path.realpath(__file__)), "log"),
            level=log_level
        )
        
        log_start(logger, "Matrix Data Process")
        # publish_message_to_sns("datateam_eks_notifications", "Starting Matrix data transfer to BigQuery")
        ...
        watchdog = Watchdog(timeout=1800, callback=watchdog_callback)

        client = create_bigquery_client()
        if client is None:
            raise Exception("Failed to create BigQuery client")
        ...
        sql_server = matrix_secrets.get(
            "MATRIX_DB_SERVER_IP_NEW"
        )  # using ip address for now
        ...
        non_unique_key_columns = {("ffx", "vw_RPT_SubsRateRounds"): "Subs_id"}

        is_date_based_processing = {"tbl_RPT_TM1retail": True}

        is_non_unique_key_based_processing = {"vw_RPT_SubsRateRounds": True}
        ...
        for schema_name, table_name in full_refresh_tables:
            try:
                logger.info(f"Starting full refresh for table: {schema_name}.{table_name}")
                load_full_refresh_data_to_bigquery(
                    cursor, client, dataset_name, schema_name, table_name
                )
                logger.info(f"Completed full refresh for table: {schema_name}.{table_name}")
            except Exception as e:
                logger.error(f"Error processing {schema_name}.{table_name}: {str(e)}")
                raise 
        ...
        for schema_name, table_name in hash_key_tables:
            try:
                logger.info(f"Starting incremental update for table: {schema_name}.{table_name}")

                dynamic_model = fetch_and_create_dynamic_model(
                    cursor, schema_name, table_name
                )
                primary_key_column = primary_key_columns.get(
                    (schema_name, table_name), None
                )
                non_unique_key_column = non_unique_key_columns.get(
                    (schema_name, table_name), None
                )
                is_date_based = is_date_based_processing.get(table_name, False)
                is_non_unique_key_based = is_non_unique_key_based_processing.get(
                    table_name, False
                )

                if is_date_based:
                    last_processed_date = get_last_processed_date(
                        client, dataset_name, schema_name, table_name
                    )
                    logger.debug(
                        f"Starting to process {table_name} with last processed date: {last_processed_date}"
                    )
                else:
                    last_processed_pk = (
                        get_last_processed_pk(
                            client, dataset_name, schema_name, table_name
                        )
                        or 0
                    )
                    logger.debug(
                        f"Starting to process {table_name} with last processed PK: {last_processed_pk}"
                    )
                ...
                temp_table_name = create_temp_table(
                    client, dataset_name, schema_name, table_name, schema
                )
                ...
                last_processed_value = (
                    ...
                    if is_date_based
                    else get_last_processed_pk(
                        client, dataset_name, schema_name, table_name
                    )
                )

                for batch_rows in load_data_from_sql_server(
                    cursor,
                    schema_name,
                    table_name,
                    primary_key_column,
                    ...
                    if processed_data:
                        logger.debug(f"Processing batch of size: {len(processed_data)}")
                        data_for_bigquery = prepare_data_for_bigquery(
                            processed_data, include_hash_key=True
                        )
                        success = upload_data_to_bigquery(
                            client, dataset_name, temp_table_name, data_for_bigquery
                        )
                        if success:
                            logger.info(
                                f"Batch uploaded successfully, number of rows: {success}"
                            )
                        else:
                            logger.error("Batch upload failed.")

                    if is_date_based:
                        last_processed_date = max(
                            row["Retail_SaleDate"] for row in processed_data
                        )
                        update_last_processed_info(
                            client,
                            dataset_name,
                            schema_name,
                            table_name,
                        ...
                        logger.debug(
                            f"Last processed date updated to: {last_processed_date}"
                        )
                    else:
                        if is_non_unique_key_based:
                            last_processed_pk = max(
                                row[non_unique_key_column] for row in processed_data
                            )
                        else:
                            last_processed_pk = max(
                                row[primary_key_column] for row in processed_data
                            )
                        update_last_processed_info(
                            client,
                            dataset_name,
                        ...
                        logger.debug(
                            f"Last processed PK updated to: {last_processed_pk}"
                        )

                logger.info(
                    f"Starting merge into final table: {table_name} from temporary table: {temp_table_name}"
                )
                update_bigquery_with_hash_keys(
                    client,
                    dataset_name,
                    schema_name,
                    table_name,
                ...
                if is_date_based:
                    ...
                    logger.debug(f"Last processed PK updated to: {last_processed_pk}")

                drop_temp_table(client, dataset_name, temp_table_name)
                logger.debug(f"Temporary table dropped: {temp_table_name}")

                watchdog.reset()
                
                logger.info(f"Completed incremental update for table: {schema_name}.{table_name}")
            except Exception as e:
                logger.error(f"Error processing {schema_name}.{table_name}: {str(e)}")
                raise  # Re-raise the exception to trigger script exit

            check_exit()  # Check if we should exit after each table
        publish_message_to_sns("datateam_eks_notifications", "Matrix data transfer to BigQuery completed successfully")
    
    except Exception as e:
        error_msg = f"An error occurred during script execution: {str(e)}"
        logger.error(error_msg)
        publish_message_to_sns("datateam_eks_notifications", error_msg)
        sys.exit(1)

...
def get_content_metadata(
    session, endpoint, path, contentid, main_pub_key, user, pwd
):
    """get the metadata of single content id"""

    full_url = (
        endpoint + path + contentid + f"?publication_channel={main_pub_key}"
    )
    ...
    try:
        ...
        ):  # 403 is usually used for access denied errors
            error_data = content.json()
            if error_data.get("code") == "AccessDenied":
                logging.info(
                    "get_content_metadata(): Access denied for content ID %s: %s",
                    contentid,
...
class LoggingHTTPAdapter(HTTPAdapter):
    ...
    def send(self, request, **kwargs):
        ...
        if retries:
            logging.info(
                "Retry attempt: %s", retries.total_retries - retries.total - 1
            )

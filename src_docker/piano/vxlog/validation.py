...
class VxSubscriptionLog(MyBaseModel):
    ...
    def coerce_to_str_phone(cls, v):  # pylint: disable=no-self-argument
        """coerce to string and remove spaces, handle NaN values"""
        if v is None:
            return None
        # Convert to string and check for NaN case-insensitive
        str_v = str(v)
        if str_v.lower() == 'nan':
            return None
        # Remove spaces and return as string
        cleaned = str_v.replace(" ", "")
